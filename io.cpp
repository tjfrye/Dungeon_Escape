...
uint32_t io_list_inventory(dungeon *d) 
{
  clear();
  uint32_t i;
  std::vector<object> &v = d->PC->inventory;

  for(i = 0; i < 10; i++) {
  	mvprintw(0, i, "Slot 0: %s", v[i].get_name());
  }

  return 0;
}

uint32_t io_list_equipment(dungeon *d)
{
  return 0;
}
...
uint32_t io_take_off_item(dungeon *d)
{
  return 0;
}
...
uint32_t io_expunge_item(dungeon *d)
{
  return 0;
}

uint32_t io_inspect_item(dungeon *d)
{
  return 0;
}
...
uint32_t io_look_monsters(dungeon *d)
{
  pair_t dest;
  int c;
  fd_set readfs;
  ...
  mvprintw(0, 0, "Choose a monster.  't' to select; 'esc' to return to game.");
  ...
  * not otherwise used.                                                 */
  ...
  if (c == 't') {
    if (charpair(dest) && charpair(dest) != d->PC) {
      ...
      n = (npc*)charpair(dest);
      clear();
      mvprintw(0, 0, "Monster Description:\n\n%s\n\nPress any key to continue.", n->description);
      getch();
    } else {
      io_queue_message("That's not a monster!");
    }
  }
